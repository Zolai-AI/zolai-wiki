# Zolai AI — 02_grammar_complete.md


---
## [complete_grammar_reference_corpus.md]
...
| `ahih leh` | but / however | `A hoih hi, ahih leh a sang hi.` = It's good, but it's expensive. |
...
|---|---|---|
| `Tua ciangin` | Then / At that time | `Tua ciangin khuavak om pah hi.` — Then light appeared. |
| `Tua ahih ciangin` | And so it was / Therefore | `Tua ahih ciangin...` — And so it was... |
| `A kipat cilin` | In the beginning | `A kipat cilin Pasian in...` — In the beginning God... |
| `Tu-in` | Now / At this point | `Tu-in gulpi pen...` — Now the serpent... |
| `ci aa [verb]` | Having said, [verb] | `Tua bang ci aa paikhia hi.` — Having said so, he left. |
| `Bang hang hiam cih leh` | Because / The reason is | `Bang hang hiam cih leh tua ni-in a tawlnga hi.` — Because on that day He rested. |

### Causal/Logical Connectors
| Zolai | English |
|---|---|
| `ahih manin` | because / therefore (cause→effect) |
| `bang hang hiam cih leh` | because (reason explanation) |
| `ahih leh` | but / however (contrast) |
| `tua ahih manin` | therefore / for that reason |
| `cih bangin` | as / like / in the manner of |

---

## 8. Focus & Topic Markers

### `pen` — Topic/Focus Marker
`pen` marks the topic or focus of a sentence (similar to Japanese `wa`):

```
Amah pen ka Pa ahi hi.     He (topic) is my Father.
Khuavak pen hoih hi.       The light (topic) is good.
Tua mi pen Laipianpa kici.  That person (topic) is called Laipianpa.
```

### `zong` — Also/Even
```
Ka zong pai ding hi.       I also will go.
Amah zong theih kei hi.    Even he doesn't know.
```

### `mah` — Emphatic/Exactly
```
Kei mah hi.                It is exactly me.
Tua mah bangin.            In exactly that way.
Hi mah hi.                 This is exactly it.
```

### Comparative Markers
| Pattern | Meaning | Example |
|---|---|---|
| `A sangin B + zaw` | B is more than A | `Khuavak lianzaw pen sun a uk ding.` — The greater light rules the day. |
| `A bang in` | Like A / as A | `Pasian bang in` — Like God |
| `A pen` | As for A (topic) | `Amah pen...` — As for him... |

---

## 9. Phonology — Extended Reference

### Consonant Clusters (Initial)
Only 4 clusters are permitted at syllable onset:
- `kh` = /x/ or /kʰ/ — `khua` (village), `khin` (past marker)
- `th` = /tʰ/ — `thu` (word/matter), `thum` (three)
- `ph` = /pʰ/ — `phung` (clan/custom)
- `ng` = /ŋ/ — `nga` (five), `ngen` (speak)

### Final Consonants (8 permitted)
`-p, -t, -k, -m, -n, -ng, -l, -h`

Examples:
- `-p`: `kap` (shoot), `sap` (call)
- `-t`: `dat` (burn), `sat` (kill)
- `-k`: `sak` (build/cause), `tak` (true)
- `-m`: `dam` (well/healthy), `lam` (way/path)
- `-n`: `van` (sky/thing), `man` (price/true)
- `-ng`: `sang` (high), `vang` (wide)
- `-l`: `kal` (go up), `tawl` (rest)
- `-h`: `muh` (see), `theih` (know)

### Minimal Pairs (Homonyms) — Critical for AI
| Word A | Meaning | Word B | Meaning |
|---|---|---|---|
| `van` | thing/goods | `vaan` | sky/heaven |
| `man` | price/true | `maan` | correct/right |
| `sang` | high/school | `saang` | to call/summon |
| `lam` | way/path/dance | `laam` | to dance (verb) |
| `zang` | to use | `zangh` | nasal variant |
| `khua` | village/place | `khuaa` | long form |
| `ni` | sun/day/two | `nii` | long form |
| `tui` | water | `tuii` | long form |

### Nasal `h` Distinction
The `h` after a nasal changes meaning:
- `zang` = to use (verb)
- `zangh` = nasal-h variant (different tone/meaning)
- Rule: `h` after nasal = distinct phoneme, not silent

### Forbidden Phonetic Combinations
1. `ti` cluster — forbidden
2. `c` + {a, e, o, aw} — `ca`, `ce`, `co`, `caw` all forbidden
3. `o` must be realized as /oʊ/ — never pure /o/ or /ɒ/

---

## 10. Stem I vs Stem II — When to Use

Verb stems alternate based on grammatical context:

| Context | Stem | Rule |
|---|---|---|
| Declarative (subject does) | Stem I | `A pai hi.` — He goes. |
| Interrogative | Stem II | `Na muh hiam?` — Do you see? |
| Nominalization (`+na`) | Stem II | `dahna` from `dah`, `liatna` from `lian` |
| Subordinate clause | Stem II | `A pai ding` — (that) he will go |
| After `ci` (quotative) | Stem I | `"Ka pai" ci hi.` |
| Imperative | Stem I | `Pai in.` — Go! |

**Common Stem II shifts:**
| Stem I | Stem II | Nominalized |
|---|---|---|
| `dah` | `dah` | `dahna` |
| `lian` | `liat` | `liatna` |
| `kap` | `kah` | `kahna` |
...
## 11. Directional Particles — Full System

These combine with verbs to specify movement direction precisely:
...
| `pih` | Send/bring (causative) | `Hong pih in.` (Bring here) | `Va pih in.` (Send away) |
| `suk` | Downward | `Suk pai hi.` (Went down) | `A tui lutsuk hi.` (Dived down) |
| `toh` | Upward | `Toh pai hi.` (Went up) | `Kahto in...` (Going up...) |

---

## 12. Locative & Spatial Particles

| Particle | Meaning | Example | Translation |
|---|---|---|---|
| `ah` | at/in/on (locative) | `Inn ah om hi.` | He is at home. |
| `sungah` | inside | `Inn sungah om hi.` | He is inside the house. |
| `tungah` | on top of / above | `Bawl tungah om hi.` | It is on the table. |
| `nuaiah` | below / under | `Bawl nuaiah om hi.` | It is under the table. |
| `lamah` | toward / in the direction of | `Nisuahna lamah` | Toward the east |
| `panin` | from | `Leitung panin` | From the earth |
| `tawh` | with / by means of | `Lai tawh` | With/by writing |
| `ding` | for (purpose) | `Ka ding` | For me |
| `sang ah` | at school / in the context of | `Sinna sang ah` | At school |

---

## 13. Politeness & Register Markers

### Softening Particles
| Particle | Function | Example |
|---|---|---|
| `ve aw` | Polite request | `Huih ve aw.` — Please help me. |
| `nuamtak` | Please / slowly | `Nuamtak gen ve aw.` — Please speak slowly. |
| `lungdam` | Thank you / gratitude | `Lungdam mahmah.` — Thank you very much. |
| `dih` | Polite prohibition | `Pai kei dih.` — Please don't go. |
| `vo` | Mild prohibition | `Pai kei vo.` — Don't go (mild). |

### Honorific Titles (used before names)
| Title | Used for |
|---|---|
| `Pu` | Respected elder man |
| `Pi` | Respected elder woman |
| `Pa` | Father / older male |
| `Nu` | Mother / older female |
| `U` | Elder sibling (gender-neutral) |
| `Nau` | Younger sibling |
| `Topa` | Lord (God — formal/biblical) |
| `Pasian` | God (standard Tedim term) |

---

## 14. Key Grammatical Terms in Zolai

| English Term | Zolai Term | Notes |
|---|---|---|
| Language | Zolai / Pau | `Zolai` = written language; `Pau` = spoken |
| Word | Kammal | `Kammal zatzia` = word rules |
| Sentence | Thu | `Thu gen` = to speak/say |
| Verb | Nasep kammal | action word |
| Noun | Min kammal | naming word |
| Pronoun | Min khelna kammal | pronoun |
| Adjective | Hoihna kammal | describing word |
| Adverb | Nasep hoihna | verb modifier |
| Particle | Zatna | grammatical particle |
| Tone | Awsaang | pitch/tone |
| Vowel | Awphei | vowel sound |
| Consonant | Laimung | consonant letter |
| Writing | Laigelh | the act of writing |
| Reading | Laisim | the act of reading |
| Grammar | Laigelhna dan | rules of writing |
| Apostrophe | Pawfi | the `'` mark |
| Plural | Tamna | many/plural |
| Singular | Khatna | one/singular |
...
### A. Parallelism
The dominant literary device in Psalms and proverbs, where the second line echoes or contrasts the first:
- `Zogam a ding Pasian, Zomi a ding Pasian` — God for Zogam, God for Zomi. *(Zogam Lapi)*

### B. Call-and-Response
- `Khapi aw hong kia, hong kia / Na koi lai na tat, na tat` *(Sinna — children's song)*

### C. Moral Framing (Gentehna Pattern)
Every parable in *Gentehna Tuamtuam* follows: **Narrative → A Deihna (Moral/Desired Meaning)**

---
*Reference: ZVS 2018, Zolai Sinna Bu (Lessons 1–34), Zolai Khanggui (pp. 12–17, 456–468), Gentehna Tuamtuam le A Deihnate.*

---

## 16. Register Variation: Formal vs. Informal

Zolai has distinct formal and informal registers, particularly in subject-verb agreement and pronoun dropping.

### Eating / Action Sentences

| Register | Example | Notes |
| :--- | :--- | :--- |
| **Formal** | `Kei in an ka ne hi.` | Full subject + ergative `in` + object + 1st person `ka` + verb + `hi` |
| **Informal** | `An ka ne hi.` | Subject (`Kei`) dropped; natural in conversation |
| **Casual** | `Ka ne hi.` | Object also dropped when context is clear |

> **Rule:** In informal/conversational speech, the explicit subject pronoun (`Kei`, `na'ng`, `Amah`) is commonly dropped when the 1st/2nd/3rd person agreement marker (`ka`, `na`, `a`) makes the subject clear from context.
>
> **For training data:** Tag informal forms with `"register": "informal"` and formal/biblical forms with `"register": "formal"`.

---

## 17. Narrative Connectors (Biblical / Story Register)

These are the backbone of Zolai storytelling, extracted from Genesis and Exodus.

| Connector | Meaning | Example |
| :--- | :--- | :--- |
| `Tua ciangin` | Then | `Tua ciangin Pasian in, "Eima' lim...mihing bawl ni," ci hi.` |
| `Tua ahih ciangin` | So when that was / Therefore | `Tua ahih ciangin Pasian in ama' lim bangin mihing piangsakin...` |
| `Tua mah bangin` | Just like that | `Tua mah bangin piang pah hi.` *(repeated in GEN 1:7, 1:9, 1:11)* |
| `Tu-in` | Now / At this point | `Tu-in gulpi pen...ngiansiam zaw hi.` |
| `Tua hun sungin` | In those days | `Tua hun sungin Kain in lei pan-a piang gah piakna...` |
| `A zing ciangin` | In the morning | `A zing ciangin amah a paikhiat ciangin...` |
| `Nitak hong bei-in, zingsang hong tung a` | Evening came, morning came | Day-counting formula in Genesis creation account |
| `A kipat cil-in` | In the beginning | `A kipat cil-in Pasian in vantung leh leitung a piangsak hi.` |

---

## 18. Quotative Patterns (Direct Speech)

Zolai direct speech always closes with `ci hi`. Mid-sentence speech uses `ci-in`.

| Pattern | Example | Translation |
| :--- | :--- | :--- |
| `"[Quote]," ci hi` | `"Khuavak om ta hen," ci hi.` | [He] said, "Let there be light." |
| `ci-in [verb]` | `Hoih hi, ci-in a mu hi.` | Seeing it, [He] said it was good. |
| `ci aa [verb]` | `Tua bang ci aa paikhia hi.` | Having said so, he left. |
| `kici` | `Laipianpa na kici hi.` | He was called Laipianpa. |
| `ci-in gen` | `"Pai in," ci-in gen hi.` | He said, "Go." |

---

## 19. Directional Verb Particles

Zolai verbs combine with directional particles to specify movement precisely.

| Particle | Direction | Example | Translation |
| :--- | :--- | :--- | :--- |
| `hong` | Toward speaker | `Hong pai in.` | Come (here). |
| `va` | Away from speaker | `Va pai in.` | Go (away). |
| `khia` | Outward / Exit | `Paikhia hi.` | He went out. |
| `lut` | Inward / Enter | `Inn sungah a lut hi.` | He entered the house. |
| `kik` | Back / Return | `Paikik hi.` | He went back. |
| `suk` | Downward | `Paisuk hi.` | He went down. |
| `pih` | Bring / Send | `Paipih hi.` | He was sent / brought. |
| `toh` | Upward | `Kahto hi.` | He went up. |

---

## 20. Blessing and Prohibition Patterns

### Blessing (thupha pia)
- `Pasian in amaute thupha pia a, "Ci pha-in tampi pha un," ci hi.` *(GEN 1:22)*
  — God blessed them, saying, "Be fruitful and multiply."
- `Pasian in ni sagih ni thupha pia-in siangthosak hi.` *(GEN 2:3)*
...
## 21. Identity Assertion: "I AM" Pattern

The most emphatic identity statement in Zolai uses full pronoun + `ka hi hi`:

- `KEIMAH ka hi hi.` *(EXO 3:14)* — I AM THAT I AM. (God's self-declaration)
- `Kei Zomi ka hi hi.` — I am Zomi.
- `Amah pen ka Pa ahi hi.` — He is my Father.
- `Keimah in na thei hi.` — You yourself know it.

Pattern: **[Emphatic pronoun] + [agreement marker] + hi hi**

---
## [pronouns_complete.md]
...
| **Perfective** | **khinsa** | Already completed before reference | `A pai khinsa hi.` — He had already gone. |
| **Resumptive** | **kik** | Do again | `A pai kik hi.` — He went again. |
| **Attempted** | **sawm** | Try to do | `A pai sawm hi.` — He tried to go. |
| **Immediate action** | **pah** | Do immediately | `A pai pah hi.` — He went immediately. |
| **Intensive** | **mahmah** | Very much | `A hoih mahmah hi.` — It is very good. |
...
- `A pai hi.` — He goes. (Simple declaration, ends with `hi`)

### B. Interrogative
- `A pai hiam?` — Does he go? (Yes/no question)
- `Koi ah pai hiam?` — Where does he go? (Content question)

### C. Imperative
| Form | Marker | Example | Translation |
|------|--------|---------|-------------|
| Direct command | **un / in** | `Pai un.` | Go! (plural) |
| Polite suggestion | **dih** | `Hong pai pak dih.` | Please come. |
| Hortative ("let us") | **ni** | `Mihing bawl ni.` | Let us make man. *(GEN 1:26)* |
| Benedictive ("let it be") | **hen** | `Khuavak om hen.` | Let there be light. *(GEN 1:3)* |
| Prohibitive | **kei in** | `Pai kei in.` | Don't go. |

### D. Optative (wish/hope)
- `Pasian thupha hong pia ta hen.` — May God's blessing come.
- `Zomite hanlung ciam khangto diam.` — May Zomi hearts grow forever. *(Sinna — ABC)*

---

## 4. Negation Patterns

| Pattern | Context | Example | Translation |
|---------|---------|---------|-------------|
| **V + lo** | General present/past | `Ka ne lo hi.` | I don't eat. |
| **V + kei** | Future/intent | `Ka pai kei ding hi.` | I won't go. |
| **V + kei + in** | Imperative negative | `Pai kei in.` | Don't go. |
| **V + keng** | 1st person contraction (kei+in) | `Ka pai keng.` | I won't go. |
| **V + ngei lo** | Experiential ("never") | `Ka mu ngei lo hi.` | I have never seen. |
| **V + thei lo** | Ability ("cannot") | `Ka gen thei lo hi.` | I cannot say. |
...
| Particle | Direction | Example | Translation |
|----------|-----------|---------|-------------|
| **hong** | Toward speaker | `Hong pai in.` | Come (toward me). |
| **va** | Away from speaker | `Va pai in.` | Go (away from me). |
| **suk** | Downward | `A tui lutsuk hi.` | He dived down. |
| **toh** | Upward | `Kahto in...` | Going up... |
| **khia** | Outward | `Paikhia hi.` | He went out. |
| **lut** | Inward | `Inn sungah a lut hi.` | He entered the house. |

---

## 6. Evidentiality & Source Markers

| Marker | Meaning | Example |
|--------|---------|---------|
| **kiza hi** | It is heard/reported | `A pai kiza hi.` — It is heard that he went. *(News register)* |
| **kigen hi** | It is said | `A hoih kigen hi.` — It is said to be good. |
| **kiummawh hi** | It is believed/suspected | `Hiding in kiummawh hi.` — It is suspected to be so. |
| **ci hi** | [Someone] says | `Topa in, "..." ci hi.` — The Lord says, "..." |
...
- This distinction preserves the "Lung-Kha" (Heart-Spirit) emotional weight in translations

---
*Reference: ZVS Bible (2018), Zolai Sinna Bu (Lessons 1–34), Zolai Khanggui (kammal zatzia, pp. 456–468), Zolai Gelhmaan Bu.*
...
# Zolai Particles & Postpositions — Corpus Verified

> Source: TDB77 Bible (28,756 verses) — frequency counts are exact
> Last updated: 2026-04-17

---

## Complete Postposition Table

| Particle | Frequency | English Equivalent | Example |
|---|---|---|---|
| `ciangin` | 8,366 | when / after (temporal) | `A tun ciangin...` = When (he) arrived... |
| `tawh` | 6,265 | with / by means of | `Topa tawh kikhawl hi` = walked with God |
| `ah` | 5,158 | in / at / into (locative) | `Jerusalem ah teeng hi` = dwelt in Jerusalem |
| `tungah` | 4,536 | to / unto / toward (dative) | `Amah tungah pia hi` = gave to him |
| `kiangah` | 3,356 | to / unto (personal dative) | `David kiangah gen hi` = said to David |
| `hangin` | 3,281 | because of / for the sake of | `Ama' hangin` = because of him |
| `bangin` | 3,184 | like / as / according to | `Topa' thu bangin` = according to the Lord's word |
| `panin` | 3,083 | from / out of | `Gam sung panin` = out of the land |
| `sungah` | 2,806 | inside / within / in the midst of | `Inn sungah` = inside the house |
| `nadingin` | 1,336 | in order to / for the purpose of | `Pai nadingin` = in order to go |
| `manin` | 1,280 | because / since (causal) | `A om lo manin` = because it does not exist |
| `mah bangin` | 674 | just as / even as | `A sih mah bangin` = just as he died |
| `kiang` | 517 | near / beside | `Topa kiang` = near the Lord |
| `ading` | 430 | for / for the sake of | `Eite ading` = for us |
| `dongin` | 339 | up to / as far as | `Ciangdong` = up to that point |
| `mateng` | 161 | until | `Ka hong pai mateng` = until I come |
| `nungah` | 100 | behind / after / following | `Ka nung hong zui un` = follow after me |
| `laizangah` | 42 | in the midst of / among | `Huan laizangah` = in the midst of the garden |
| `kiimah` | 9 | around / round about | `Gibeah kiimah` = round about Gibeah |

---

## Detailed Usage

### `tawh` — with / by means of / instrument
- `Topa tawh kikhawl hi` = walked with God
- `Namsau tawh thah hi` = killed with a sword
- `Kampau tawh ahi zongin` = not only with words
- `Hehpihna tawh` = with grace/kindness

### `ah` — in / at / into (locative)
- `Jerusalem ah teeng hi` = dwelt in Jerusalem
- `Inn sungah lut hi` = entered into the house
- `Vantungah` = in heaven
- `Leitungah` = on earth

### `tungah` — to / unto (dative, things/places)
- `Amah tungah pia hi` = gave to him
- `Topa tungah thunget hi` = prayed to the Lord
- `Note tungah gen hi` = said to you (all)

### `kiangah` — to / unto (personal, direct address)
- `David kiangah gen hi` = said to David
- `Amaute kiangah` = to them
- `Ka kiangah` = to me

### `hangin` — because of / for the sake of / through
- `Ama' hangin` = because of him / for his sake
- `Pasian' hangin` = for God's sake
- `Khazih hangin` = through Christ

### `bangin` — like / as / according to
- `Topa' thu bangin` = according to the Lord's word
- `A sih mah bangin` = just as he died
- `Ka gamtat mah bangin` = as I do

### `panin` — from / out of
- `Gam sung panin` = out of the land
- `Ama' tung panin` = from him
- `Vantung panin` = from heaven

### `sungah` — inside / within / in the midst of
- `Inn sungah` = inside the house
- `Biakinn sungah` = inside the temple
- `Huan laizangah` = in the midst of the garden (laizangah = in the middle)

### `nadingin` — in order to / for the purpose of
- `Pai nadingin` = in order to go
- `Hotkhiat nadingin` = in order to save
- `Theih nadingin` = in order to know

### `manin` — because / since (causal suffix)
- `A om lo manin` = because it does not exist
- `Ahih manin` = because it is so (most common: 258 occurrences)
- `Ka it manin` = because I love

### `ading` — for / for the sake of
- `Eite ading` = for us
- `Amah ading` = for him
- `Pasian ading` = for God

### `mateng` — until
- `Ka hong pai mateng` = until I come (78 occurrences)
- `A dam mateng` = until he recovers
- `Tua hun mateng` = until that time

---

## Imperative Forms

| Ending | Meaning | Frequency | Example |
|---|---|---|---|
| `[verb] in` | imperative singular | 666 | `Pai in.` = Go! |
| `[verb] un` | imperative plural | 550 | `Pai un.` = Go! (you all) |
| `[verb] hen` | optative (let it be) | 203 | `Khuavak om hen.` = Let there be light. |
| `[verb] ni` | hortative (let us) | 40 | `Pai ni.` = Let us go. |
| `[verb] tahen` | wish (may it be) | — | `Hong kem tahen.` = May he keep you. |

### Imperative examples (corpus-verified)
- `Pai in.` = Go! (singular)
- `Pai un.` = Go! (plural)
...
## Possession Marker: Apostrophe `'`

Possession shown by `'` (apostrophe/pawfi) after the possessor:
- `Pasian' inn` = God's house
- `David' tapa` = David's son
- `ama' zi` = his wife
- `ka' lungsim` = my heart (often written `ka lungsim`)
- `amau' omna` = their dwelling

**Rule:** Apostrophe marks possession. Different from contraction (`na'ng` = you + going).
...
# Zolai Questions & Answers — Everyday Reference

> Sources: parallel_combined (conversational), Bible (TDB77/TBR17/Tedim2010), wiki
> Last updated: 2026-04-17

---

## Greetings & Basic Exchanges

| English | Zolai |
|---|---|
| How are you? | `Na dam na?` |
| I am fine. | `Ka dam hi.` |
| Are you well? | `Na dam hiam?` |
| Peace be with you. | `Nuam takin om un.` |
| Go in peace. | `Nuam takin pai in.` |
| Thank you. | `Lawmthu ka nei hi.` |
...
| We went. | `I pai hi.` |
| He went. | `Amah pai hi.` |
| She came. | `Amah hong pai hi.` |
| I do not know. | `Ka thei kei hi.` |
| I want to go. | `Ka pai nuam hi.` |
...
| English | Zolai |
|---|---|
| How are you? | `Na dam na?` / `Na dam hiam?` |
| Who are you? | `Nang kua na hi hiam?` |
| What is this? | `Hih bang ahi hiam?` |
| Where are you going? | `Koi lam pai na hi hiam?` |
| How old are you? | `Na nuntak kum bangzah pha ta ahi hiam?` |
| Who are these? | `Hihte kua ahi hiam?` |
| Will you go? | `Na pai ding hiam?` |
| Do you understand? | `Na theih hiam?` |
| Is God good? | `Pasian hoih ahi hiam?` |
| Where is it? | `Koi lai-ah om hiam?` |
| Why did you do that? | `Bang hangin tua bangin gamta na hi hiam?` |
| What do you want? | `Bang deih na hi hiam?` |
| Can you come? | `Hong pai thei na hi hiam?` |
...
|---|---|
| Yes / It is so. | `Ahi hi.` |
| No / It is not so. | `Hilo hi.` |
| Yes, I will go. | `Ahi hi, ka pai ding hi.` |
| No, I will not go. | `Hilo hi, ka pai kei ding.` |
| I can. | `Ka thei hi.` |
| I cannot. | `Ka thei kei hi.` |
| I don't know. | `Ka thei kei hi.` |
| Not yet. | `Nai lo hi.` |
| No more. | `Nawn kei hi.` |
...
| my son | `ka tapa` |
| my daughter | `ka tanu` |
| my brother | `ka sanggampa` |
| my sister | `ka sanggamnu` |
| my wife | `ka zi` |
| my husband | `ka pasal` |
| my child | `ka naupang` |
| My father reads a book. | `Ka pa in laibu a sim hi.` |
...
## Question Structures (Summary)

| Pattern | Meaning | Example |
|---|---|---|
| `[verb] hiam?` | Did/does/will...? | `Na pai hiam?` = Did you go? |
| `[verb] khin hiam?` | Have you (already)...? | `Na pai khin hiam?` = Have you gone? / Did you go? |
| `[verb] ngei hiam?` | Have you ever...? | `Na mu ngei hiam?` = Have you ever seen (it)? |
| `ahi hiam?` | Is it...? | `Pasian kua hiam?` = Who is God? |
| `ding hiam?` | Will...? | `Hong pai ding hiam?` = Will (he) come? |
| `om hiam?` | Is there...? | `Koi lai-ah om hiam?` = Where is it? |
| `thei hiam?` | Can...? | `Na thei hiam?` = Do you know? |
| `bang...hiam?` | What...? | `Bang ahi hiam?` = What is it? |
| `bangci...hiam?` | How...? | `Bangci na hi hiam?` = How are you? |
| `kua...hiam?` | Who...? | `Kua in...hiam?` = Who...? |
| `bang hangin...hiam?` | Why...? | `Bang hangin tua bangin...hiam?` |
| `koi lai-ah...hiam?` | Where...? | `Koi lai-ah om hiam?` = Where is it? |

### Past tense questions

| English | Zolai | Notes |
|---|---|---|
| Did you go? | `Na pai hiam?` | simple past question |
| Did you go? (completed) | `Na pai khin hiam?` | `khin` = already done |
...
---
## [advanced_patterns_corpus.md]

# Zolai Advanced Grammar Patterns — Corpus Verified

> Source: TDB77 + TBR17 + Tedim2010 × KJV — 85,124 pairs
> Last updated: 2026-04-17
...
Zolai has no relative pronoun (who/which/that). Instead, a participial phrase precedes the noun.

**Pattern:** `a [verb] mi` = the one who [verb] / those who [verb]

| Zolai | English | Count |
|---|---|---|
| `a om mi` | the one who is / those who are | 18 |
| `a gamta mi` | the one who does | 8 |
| `a ngah mi` | the one who receives | 8 |
| `a nei mi` | the one who has | 7 |
| `a teeng mi` | the one who dwells | 6 |
| `a mu mi` | the one who sees | 3 |
| `a siangtho mi` | the one who is holy | 3 |
| `mi khat` | a certain person / someone | 77 |

**Examples (corpus-verified):**
- `Kei hong itte ka hausak a...` = He enriched those who love me
- `A om khempeuh in...` = All who are present...
- `A pai mi in...` = The one who went...
- `Topa a it mi in a u a nau a it ding hi.` = He who loves God must love his brother.

**For things:** `a [verb] thu` = the thing that [verb]
- `A bawlsa thu` = the thing he made
- `A gen thu` = what he said

---

## 2. Serial Verbs (Verb Chaining)

Zolai chains verbs using `-in` or `a` between them. The first verb is subordinate, the second is main.

**Pattern:** `[verb1]-in [verb2] hi` = [verb1] and then [verb2]

| Serial form | Count | Meaning |
|---|---|---|
| `pai-in` | 499 | went and... |
| `kizui-in` | 244 | following and... |
| `pia-in` | 159 | giving and... |
| `khia-in` | 126 | going out and... |
| `kisai-in` | 119 | concerning and... |
| `zui-in` | 108 | following and... |
| `nei-in` | 81 | having and... |
...
**Rule:** `-in` suffix on first verb = "and then" / sequential action.

---

## 3. Topic Marker: `tawh kisai` / `kisai-in`

**Meaning:** concerning / regarding / about / as for

- `Topa' min tawh kisai-in Solomon' minthan'na a za hi.` = She heard of Solomon's fame concerning the Lord's name.
- `Judah gam leh Jerusalem khua tawh kisai Isaiah muhna thu.` = The word Isaiah saw concerning Judah and Jerusalem.
- `Hih thu tawh kisai-in...` = Concerning this matter...
...
- `Sihna aw, na zawhna koi lai-ah om hiam?` = O death, where is your sting?
- `Ka ta aw` = O my son
- `Ka itte aw` = O my beloved (1 John pattern)
- `Topa aw` = O Lord
- `Korin mite aw` = O Corinthians

**Pattern:** `[addressee] aw,` = O [addressee],

---

## 5. Reciprocal: `khat leh khat` / `ki-` prefix

**`khat leh khat`** = one another / each other
- `Khat leh khat ki-it ni.` = Let us love one another.
- `Khat leh khat a siangtho tawh hopih un.` = Greet one another with a holy kiss.
- `Eite khat leh khat ki-it leh...` = If we love one another...

**`ki-` prefix** = reciprocal/reflexive action
- `kikhawm` = gather together
- `kimu` = meet each other
- `kiteeng` = marry / live together
- `kikhen` = separate from each other
- `kiho` = talk with each other
- `ki-it` = love one another

---

## 6. Distributive: `khempeuh` / `khatpeuh`

| Zolai | Meaning | Example |
|---|---|---|
| `khempeuh` | all / every / each | `mi khempeuh` = all people |
| `khatpeuh` | every single one | `khua khatpeuh` = every village |
| `a om khempeuh` | all who are | `a om khempeuh in` = all who are present |
| `na khempeuh` | all yours | `na nasep khempeuh` = all your work |
...
## 7. Intensifier: `mahmah`

**Meaning:** very / greatly / exceedingly

- `Lian mahmah hi.` = Very great.
- `Hoih mahmah hi.` = Very good.
- `Heh mahmah hi.` = Very angry.
- `Huaiham mahmah hi.` = Very zealous.
- `Tangvalte' khialhna lian mahmah hi.` = The sin of the young men was very great.

**Pattern:** `[adjective/verb] mahmah hi` = very [adj/verb]

**Other intensifiers:**
- `takpi` = truly / really: `nakpi takin` = very severely
- `taktak` = truly/really (emphatic): `Ta taktak ahi napi-in` = Although truly a Son
- `zenzen` = absolutely/completely: `Tua bang i ci zenzen kei ding hi` = May it absolutely never be

---

## 8. Approximative: `bang`

**Meaning:** about / approximately

- `Ni sawm bang ciangin` = After about ten days
- `Kum thum bang` = About three years
- `Mi za bang` = About a hundred people

**Pattern:** `[number] bang` = about [number]

---

## 9. Possession Patterns

**Apostrophe `'`** marks possession:
- `Pasian' inn` = God's house
- `David' tapa` = David's son
- `ama' zi` = his wife
- `amau' omna` = their dwelling
...
|---|---|---|
| 1sg | `ka` | `ka pa` = my father |
| 2sg | `na` | `na inn` = your house |
| 3sg | `ama` / `ama'` | `ama' tapa` = his son |
| 1pl | `i` / `eite` | `i Pasian` = our God |
| 2pl | `na` / `note` | `na upna uh` = your faith |
| 3pl | `amau` / `amau'` | `amau' gam` = their land |

---

## 10. Common Verb Compounds

| Zolai | Meaning | Components |
|---|---|---|
| `paikhia` | go out / depart | pai (go) + khia (out) |
| `paisan` | go up / ascend | pai (go) + san (up) |
| `paitawn` | go down / descend | pai (go) + tawn (down) |
| `paikik` | go back / return | pai (go) + kik (back) |
| `kuankhia` | go out | kuan (exit) + khia (out) |
| `kuansan` | go up | kuan + san |
| `suakkhia` | come out / emerge | suak (emerge) + khia |
| `ciahkik` | return / come back | ciah (arrive) + kik (back) |
| `hotkhia` | save / rescue | hot (rescue) + khia (out) |
| `damsak` | heal / make well | dam (well) + sak (causative) |
| `nungtasak` | give life / make live | nungta (live) + sak |
| `thahsak` | kill / cause to die | thah (kill) + sak |

---

## 11. Sentence Patterns Summary

### Simple statement
`[S] in [O] [V] hi.` = Subject does Object.
`Ka pa in laibu a sim hi.` = My father reads a book.

### Identity
`[S] pen [N] ahi hi.` = Subject is Noun.
`Amah pen siampi khat ahi hi.` = He is a priest.

### Existence
`[S] [place] ah om hi.` = Subject exists at place.
`Pasian vantungah om hi.` = God is in heaven.

### Relative clause
`a [V] mi in [main clause]` = The one who V does...
`A it mi in a u a nau a it ding hi.` = He who loves must love his brother.

### Serial verb
`[V1]-in [V2] hi.` = Did V1 and then V2.
`Hong pai-in gen hi.` = Came and said.

### Purpose
`[V1] nadingin [V2] hi.` = V2 in order to V1.
`Hotkhiat nadingin Tapa hong khal hi.` = Sent the Son in order to save.

### Conditional
`[condition] leh, [result] ding hi.` = If condition, result will happen.
`Na it leh, na thupha ngah ding hi.` = If you love, you will receive blessing.

---
## [agreement_markers_corpus.md]

# Zolai Agreement Markers & Plural System — Corpus Verified

> Source: TDB77 Bible (28,756 verses) — frequency counts exact
> Last updated: 2026-04-17

---

## Agreement Marker System

Zolai uses **agreement prefixes** on verbs (not pronouns) to mark person.

| Person | Prefix | Example | Meaning |
|---|---|---|---|
| 1st singular | `ka` | `ka pai hi` | I go |
...
## Plural Marker `uh` — Frequency Data

`uh` appears after the verb to mark plural subject (2nd or 3rd person):
...
## 1st Person Plural: `i` vs `ka uh`

- `i` = we (subject prefix, 1st plural inclusive) — 49 occurrences
- `ka uh` = we (1st plural, rare variant) — only 1 occurrence
- **Standard form:** `i pai hi` = we go/went
...
## `tua` — Discourse Connector (Most Common Sentence Starter: 5,678)

`tua` = that / then / so (demonstrative + discourse marker)

| Form | Count | Meaning |
|---|---|---|
| `tua ciangin` | 3,703 | then / after that |
| `tua ahih ciangin` | 2,035 | therefore / so |
| `tua lai` | 683 | at that time / there |
| `tua bang` | 399 | like that / in that way |
| `tua bangin` | 278 | in that manner |
| `tua thu` | 237 | that word/matter |
| `tua ni` | 235 | that day |
| `tua mah` | 172 | that very (emphatic) |
| `tua hun` | 140 | at that time |
| `tua khit` | 126 | after that |

---

## Nominalization: `-na` Suffix (Stem II)

Adding `-na` to a verb creates a noun (action → concept):

| Verb (Stem I) | Noun (Stem II) | Meaning |
|---|---|---|
| `biak` | `biakna` (1,538) | worship → worship/prayer |
| `mawh` | `mawhna` (702) | sin → sin/fault |
| `hehpih` | `hehpihna` (378) | be gracious → grace |
| `gim` | `gimna` (378) | suffer → suffering/trouble |
| `nungta` | `nungtakna` (374) | live → life |
| `khial` | `khialhna` (351) | err → error/sin |
| `lungdam` | `lungdamna` (330) | be glad → joy/gladness |
| `heh` | `hehna` (315) | be angry → anger |
| `siat` | `siatna` (305) | destroy → destruction |
| `ngeina` | `ngeina` (304) | custom → custom/habit |
| `pil` | `pilna` (302) | be wise → wisdom |
| `vangliat` | `vangliatna` (297) | be mighty → power/might |
| `si` | `sihna` (296) | die → death |
| `kikhop` | `kikhopna` (275) | gather → gathering/assembly |
| `gamtat` | `gamtatna` (270) | do/act → action/deed |
| `pai` | `paina` | go → going/journey |
| `gen` | `genna` | say → saying/speech |
| `thei` | `theihna` | know → knowledge |
| `om` | `omna` | exist → existence/dwelling |

**Rule:** Use Stem II (`-na`) for:
- Nominalization: `paina` = the going / journey
- Purpose clauses: `pai nadingin` = in order to go
- Subordinate clauses: `a pai ciangin` = when he went

---

## Common Agreement Marker Patterns

### 1st person singular (`ka`)
```
ka pai hi     = I go/went
ka thei hi    = I know/can
ka nei hi     = I have
ka om hi      = I am/exist
ka gen hi     = I say/said
ka it hi      = I love
ka lum hi     = I sleep
...
```
na pai hi     = you go/went
na thei hiam? = do you know?
na dam hiam?  = are you well?
na gen hi     = you say/said
na om hi      = you are/exist
```

### 3rd person singular (`a`)
```
a pai hi      = he/she goes/went  (2,994 occurrences of "a ci")
a gen hi      = he/she says/said
a nei hi      = he/she has
a bawl hi     = he/she does/did
a tun hi      = he/she arrives/arrived
a si hi       = he/she dies/died
a om hi       = he/she is/exists
```
...
## They vs We — Critical Distinction
...
# Zolai Subordinate Clauses & Connectors — Corpus Verified

> Source: TDB77 Bible (28,756 verses)
> Last updated: 2026-04-17

---

## 1. Temporal Clauses (Time)

### When: `ciangin` (1,740 occurrences)
- `A tun ciangin...` = When (he) arrived...
- `A nek khit ciangin...` = After eating...
- `Zingsangin David a thawh ma-in...` = Before David rose in the morning...

### While: `lai takin` / `laitakin`
- `Kumpipa tawh kiho-in a om lai takin Nathan hong tum hi.` = While she was talking with the king, Nathan came in.
- `Ka om lai takin...` = While I was there...

### After: `khit ciangin`
- `A nek khit ciangin...` = After eating...
- `Ni tampi khit ciangin...` = After many days...
- `Tua khit ciangin...` = After that...

### Before (time): `ma-in`
- `Ama hun ma-in` = before his time
- `Ka sih ma-in` = before I die
- `Zingsangin David a thawh ma-in` = before David rose in the morning
- `Tu ma-in` = previously / before now

### Before (space/presence): `mai-ah`
- `Topa mai-ah` = before the Lord / in the Lord's presence
- `Kumpipa mai-ah` = before the king
- `Topa' biakna tau mai-ah` = before the altar of the Lord

**Key distinction:**
- `ma-in` = before in **time** (temporal)
- `mai-ah` = before in **space/presence** (locative)

### Until: `mateng` (78 occurrences)
- `Ka hong pai mateng` = until I come
- `A dam mateng` = until he recovers
- `A bei mateng` = until it subsides

### Up to a point: `ciangdong` / `dongin`
- `Ciangdong` = up to that point / as far as
- `Tua hun ciangdong` = up to that time

---

## 2. Manner Clauses (How)
...
### According to: `bangin`
- `Topa' thu bangin` = according to the Lord's word
- `A kisam bangin` = as appointed / as arranged
...
## 4. Exception & Exclusion

### Only: `bek`
- `Moses bek in hong nai ding hi.` = Only Moses shall come near.
- `Note bek tawh a bawl hi lo.` = Not with you only.
- `Khat bek` = only one
- `Keimah bek` = only me / I alone

### Except / Save: `longal`
- `Kha Siangtho longal kuamah in a thei kei hi.` = No one knows except the Holy Spirit.
- `Zeisu Khazih longal` = except Jesus Christ

### Nothing / No one: `kuamah` + `kei`
- `Kuamah in a thei kei hi.` = No one knows.
- `Bangmah nei het lo hi.` = Has nothing at all.

---

## 5. Addition & Contrast

### Not only...but also: `tua tham loin...zong`
- `Tua tham loin, sumpi keempa in lungsim dik a nei ding hi.` = Moreover, a steward must be found faithful.
- `Tua tham loin...zong` = Not only that...but also

### Furthermore / Moreover: `tua tham loin`
- `Tua tham loin amah in Topa' thuciamna singkuang mai-ah na semin...` = Moreover he appointed Levites to minister before the ark.

### Also / Too: `zong`
- `Kei zong ka pai ding hi.` = I also will go.
- `Amaute zong...` = They also...
- `Tua tham loin...zong` = Not only...but also

### But / However: `ahih hangin` (912 occurrences)
- `Ahih hangin ka pai kei hi.` = But I did not go.
- `Ahih hangin mual tunga biaknate kila khia lo hi.` = But the high places were not removed.

### Although / Even though: `napi` / `napi-in`
- `Ta taktak ahi napi-in gimna tawlna a thuakna tawh thuman'na a sin hi.` = Although truly a Son, he learned obedience through suffering.
- `Tu-in na tampi a om napi-in pumpi khat bek a om hi.` = Now there are many members, yet only one body.
...
| English | Zolai | Notes |
|---|---|---|
| when | `ciangin` | most common (1,740) |
| while | `lai takin` / `laitakin` | simultaneous |
| after | `khit ciangin` | sequential |
| before (time) | `ma-in` | temporal |
| before (space) | `mai-ah` | locative/presence |
| until | `mateng` | (78) |
| just as | `mah bangin` | manner |
| so that | `ahih manin` | result |
| only | `bek` | exclusion |
| except | `longal` | exception |
| not only...but also | `tua tham loin...zong` | addition |
| moreover | `tua tham loin` | addition |
| also/too | `zong` | addition |
| but/however | `ahih hangin` | contrast (912) |
| although | `napi` / `napi-in` | concessive |
| very | `mahmah` | intensifier |
...
Verbs are modified by particles that attach to the stem:

| Particle | Meaning | Example |
| :--- | :--- | :--- |
| **-sak** | Causative (make/let) | `paisak` (make go), `simsak` (make read) |
| **-pih** | Comitative (with) | `paipih` (go together), `sempih` (work with) |
| **-na** | Instrument/Place | `dawnna` (cup — thing for drinking) |
| **-nading** | Future purpose | `painading` (for the purpose of going) |
| **-pak** | Immediate/temporary | `paipak` (go just now) |
| **-dih** | Polite request | `paidih` (please go) |
| **-kik** | Repetitive (again) | `paikik` (go again) |
| **-khia** | Outward | `paikhia` (go out) |
| **-lut** | Inward | `pailut` (go in/enter) |
| **-suk** | Downward | `lutsuk` (dive down) |
| **-toh** | Upward | `kahtoh` (climb up) |
| **-khawm** | Together | `sepkhawm` (work together) |
| **-sawm** | Attempt | `paisawm` (try to go) |

---

## 6. Suffix Standardization
...
- `-nading` (purpose): `painading` (in order to go)
- `-pa/-nu` (person): `siampa` (teacher), `sianu` (female teacher)

---

## 7. Linguistic Restrictions (Forbidden Patterns)

Based on *Zolai Sinna Bu* (lines 1074–1076):
1. **[t] + [i]**: These sounds do not combine in Zolai.
2. **[c] + [a, e, o, aw]**: The initial consonant `c` only pairs with `i` and `u`.
3. **[uh] + [i]**: The plural marker `uh` never co-occurs with 1st person inclusive `i`.
4. **-na / -nading:** Always join to the Stem II form (e.g., `sepna`, `hinnading`).
5. **Occupation Titles:** Always join the title to the noun (e.g., `Hausapa`, `Siavuanpa`, `Lokhote`).
6. **Clan & Place Names:** Always treat as a single joined word (e.g., `Hatlang`, `Tedim`, `Tualzang`).
7. **Complex Compounds:** Use a hyphen for clarity in long compounds (e.g., `Humpi-nelkai`).

---
*Reference: Zolai Gelhmaan Bu (Vol I, Sections 153–221, 773–775), Zolai Sinna Bu (Lessons 1–34), Zolai Khanggui (pp. 12–17, 429–500).*
...
## Core Rule
Whenever a subject performs an action on an object, the subject must be marked with **"in"**.

### Standard Transitive Pattern: [Subject] + **in** + [Object] + [Verb]

| Type | Zolai Sentence | English Translation |
|------|----------------|---------------------|
| **Transitive** | **Amah in** laibu sim hi. | **He** reads the book. |
| **Intransitive** | Amah pai hi. | He goes. (No "in" needed) |

## Pronoun Merging (Contractions)
In spoken and modern written Zolai (ZVS), the ergative "in" often merges with pronouns to create a single, powerful agentive form.

| Root Pronoun | + "in" (Ergative) | Resulting Form | Example |
|--------------|------------------|----------------|---------|
| **Kei** (I) | Kei + in | **Ken** | **Ken** ka sim hi. (I read it.) |
| **na'ng** (You) | na'ng + in | **Nan** | **Nan** na sim hi. (You read it.) |
| **Ama** (He/She) | Ama + in | **Aman** | **Aman** a sim hi. (He/She reads it.) |
| **Eite** (We) | Eite + in | **Eiten** | **Eiten** i sim hi. (We read it.) |

## 2018 Standard Compliance (ZVS)
The **2018 Zokam Standard Version (ZVS)** strictly enforces these markers to prevent ambiguity between the subject and the object, especially in complex theological texts.

- **Incorrect:** "Amah laibu sim hi." (Ambiguous; lacks agency marker)
- **Correct:** "**Amah in** laibu sim hi." (Clear agentive action)

## Semantic Nuance
- **Emphasis:** Using the merged form (**Ken**, **Nan**) puts stronger emphasis on the agent's responsibility for the action.
- **Formal vs. Informal:** In very formal or biblical contexts, the split form (**Kei in**, **na'ng in**) is sometimes used for rhythmic or traditional weight, but the merged forms are the standard for the AI Second Brain's active generation.
...
### A. Possessive Contraction
Contracted from the full possessive particle `ii`.
- **Pasian' thu** (God's word) instead of *Pasian ii thu*.
- **Ama' ta** (His child) instead of *Amah ii ta*.

### B. Agentive/Ergative Omission
Used when the agentive particle `in` is attached to a pronoun or noun ending in a vowel, omitting the 'i'.
- **Kote'n** (We [exclusive] + agentive) instead of *Kote in*.
- **Amah'n** (He/She + agentive) instead of *Amah in*.

### C. Word Distinction (Homonym Disambiguation)
Distinguishes words with similar spellings but different roots or meanings.
- **thuman'na** (faithfulness/action) vs. **thumanna** (truth/abstract).

---

## 2. Spacing and Compound Words

Consistent with *Zolai Khanggui (AD 1899–2013)* lines 483–500, Zolai follows the **"One Concept = One Word"** (`noun khat word khat`) rule.

### A. Single Concept Compounds
Join words that represent a single, unified idea:
- **Vantun** (Heaven) — *van* (sky) + *tung* (above)
- **Leitung** (Earth) — *lei* (ground) + *tung* (above)
- **Tuipi** (Sea) — *tui* (water) + *pi* (big)
- **Lai Siangtho** (Holy Bible) — Standardized specific capitalization.
- **Innkuan** (Family) — *inn* (house) + *kuan* (gather/group)
- **Mihing** (Human) — *mi* (person) + *hing* (alive)

### B. Logical Joining
- **Plurality:** Suffixes `-te` and `-teng` are joined: `mite`, `naupangteng`.
- **Nominalization:** Suffix `-na` is joined to the preceding verb: `itna`, `theihna`.
- **Reciprocal:** Prefix `ki-` is joined: `kipawl`, `kituak`.

---

## 3. Double Vowels & Tones
- **Double Vowels:** Used for long tones or distinct vowel lengths (e.g., `vaan` for sky, `van` for goods).
- **Phonetic Realization:** The letter `o` is realized as /oʊ/. Doubling `o` is rare; usually, `u` is used for the /u/ sound.

---

## 4. Diminutives
- Names and objects often take the suffix `-no` for endearment or smallness.
- Examples: **Minno**, **Tuangno**, **Aksi** (star — bird-die/seed-small).

---

## 5. Punctuation Marks (Laimal Zatzia)
1. **Tawpna (.)**: Period for ends of sentences.
2. **Husanna (,)**: Comma for lists. **Note:** ZVS forbids a comma before `le` (and).
   - ✅ `Thangpi, Lunsen le Lian Pau`
3. **Ngakna (:)**: Colon for lists or direct quotes.
4. **Thekna (-)**: Hyphen for ranges or specific complex compounds.

---
*Reference: ZVS Standard Format (2018), Zolai Khanggui (Lines 429–500), Zolai Sinna Bu.*
...
## 1. Tone Categories (Pitch)

The pitch of a word in Zolai changes its meaning. There are 4 primary pitch levels:

1. **P1 (High)**: A rising or high-level pitch (similar to an English question).
2. **P2 (Mid-High)**: A steady pitch above the baseline.
3. **P3 (Mid-Low)**: The baseline or normal speaking pitch.
4. **P4 (Low)**: A low or falling pitch.

---

## 2. Length Categories (Vowel Duration)

Vowel length is critical and is marked orthographically in the **Zolai Standard Format (ZVS)**:

- **Short (S)**: Single vowel (e.g., `an` - food).
- **Long (L)**: Double vowel (e.g., `aan` - to lean).

---

## 3. Tonal Minimal Pairs

The combination of pitch and length (Tonemes) creates distinct meanings for identical consonant-vowel combinations.

| Word | Tone/Length | Meaning | Example |
| :--- | :--- | :--- | :--- |
| **van** | P3 Short | Thing / Goods / Object | `Hih van bang hi hiam?` |
| **vaan** | P3 Long | Sky / Heaven | `Vantung ah Pasian om hi.` |
| **sa** | P3 Short | Meat / Flesh | `Bawng sa ka ne hi.` |
| **saa** | P1 Long | Hot / To feel hot | `Tui a saa mahmah hi.` |
| **gu** | P4 Short | Bone | `Ka gu a na hi.` |
| **guu** | P4 Long | To steal | `Van a guu hi.` |

---

## 4. Orthographic Tone Markers (Symbols)

While double vowels mark length, Zolai also uses punctuation-like markers for specific tonal or grammatical shifts:

- **Double Vowels (`aa`, `ee`, `ii`, `oo`, `uu`)**: Marks long vowels (P1L, P2L, etc.).
- **Apostrophe (`'`)**: Often marks a glottal stop or a clipped tone at the end of a syllable (e.g., `pa'` in `ka pa'`).
- **Comma (`,`)**: Used in some dictionaries to mark rising pitch.
- **Full Stop (`.`)**: Used in some dictionaries to mark falling pitch.

---

## 5. Phonetic Restrictions (ZVS Rules)

Regardless of tone, Zolai follows strict phonetic rules:
- **No `ti` clusters**: The sounds `t` and `i` never combine. Use `te` or `tu` equivalents.
- **`c` Restrictions**: The `c` consonant only pairs with `i` and `u`. Words like `ca`, `ce`, `co`, `caw` are strictly forbidden.
- **`o` Realization**: The letter `o` is always pronounced as a diphthong `/oʊ/` (like "go").

---

## 6. Tone Sandhi (Tone Shifting)

In Zolai, the tone of a word can shift depending on the following word, especially with particles:
- **Declarative `hi`**: Often pulls the preceding verb into a neutral or mid-tone.
- **Interrogative `hiam`**: Forces a rising tone on the final syllable.

---
*Reference: Zolai Gelhmaan Bu, ZVS Standard (2018).*
...
## 1. Primary Aspect Markers

| Aspect | Particle | Function | Example | Translation |
| :--- | :--- | :--- | :--- | :--- |
| **Simple/Statative** | `hi` | General fact or state | `A pai hi.` | He goes / He went. |
| **Progressive** | `laitak` | Ongoing action (now) | `A pai laitak.` | He is going. |
| **Immediate Past** | `ta` | Just happened | `A pai ta.` | He has gone (just now). |
| **Completed Past** | `khin` | Finished some time ago | `A pai khin hi.` | He has already gone. |
| **Resultative** | `zo` | Action finished/achieved | `A pai zo hi.` | He has finished going. |
| **Future** | `ding` | Intention or future fact | `A pai ding hi.` | He will go. |
| **Experiential** | `ngei` | "Ever" or "Used to" | `A pai ngei hi.` | He has gone before. |

---

## 2. Negation and Aspect

Zolai uses specific particles to negate different temporal states.

- **`kei` (Simple Negation)**: Used for future, habitual, or potential negation.
  - `Ka pai kei ding hi.` (I will not go.)
...
- **`nawn lo` (No longer)**: Negates an ongoing state or habit.
  - `A pai nawn lo hi.` (He no longer goes.)

---

## 3. Directional Influence on Aspect

Directional particles often imply an completed or resultative aspect:
- `khia` (out): `Pulaakkhia` (to have published/released)
- `to` (up): `Kahto` (to have ascended)
- `suk` (down): `Paisuk` (to have descended)

---

## 4. The "uh" Redundancy Rule (Critical)
...
## 5. Potential Aspect (`theih`)

To express ability or permission, use `theih`:
- `Ka pai theih hi.` (I can go.)
- `Ka pai thei kei hi.` (I cannot go.)

---
*Reference: ZVS 2018 Corpus, Zolai Gelhmaan Bu.*

---
## [neologism_morphology.md]
...
## Key Dialects in the Corpus

| Dialect | Description | Relationship to Standard |
|---------|-------------|--------------------------|
| **Tedim** | The most widely spoken and the basis for the literary standard. | Primary source for modern ZVS. |
| **Sizang** | Known for its distinct "Sizang" tone and specific vocabulary (e.g., using "Ka" more frequently). | Influences historical texts (1932/1977). |
| **Falam (FCL)** | A closely related Chin language with overlapping vocabulary but distinct grammar. | Used for cross-linguistic reference. |
| **Hakha (HCL)** | Another major Chin language, more distinct from Tedim but shares cultural "Lung-Kha" roots. | Used for comparative theological mapping. |

## Notable Differences

### 1. Pronoun and Particle Usage
- **Standard (ZVS):** Uses "in" for ergative marking and "hi" for present tense.
- **Sizang Influence:** Sometimes omits certain final particles or uses alternative future markers like "suak" in specific contexts.

### 2. Phonetic Shifts in Spelling
- **Archaic Tedim:** Often uses 's' where modern Zokam might use 'sh' in certain transliterations (e.g., *Izipt* vs. *Egypt*).
- **Burmese Influence:** Older dialects (1932) reflect phonetic borrowings from Burmese that the 2018 Standard has moved away from in favor of more native or international phonetic patterns.

### 3. "Lung" (Heart) Idioms
The way "Lung" is paired with verbs varies slightly across dialects, though the core "Lung-Kha" architecture remains universal among the Zomi people.

- **Tedim:** *Lungdam* (Happy).
- **Comparative:** Some dialects may use different modifiers to express the same "Heart-state," which the AI Second Brain must map correctly to maintain tone.

## Multi-Version Training Strategy
To handle these nuances, the **Zolai AI Second Brain** uses:
1. **ZVS (2018)**: For normative grammar and modern formal output.
2. **TDB77 (1977)**: For traditional church and community register.
3. **FCL/HCL (Chin-Bible)**: For identifying cross-dialectal patterns and ensuring broader accessibility across the Chin State.
...
# Zolai Prepositions & Postpositions Reference — Corpus Verified
...
- `[TDB77]` *And they dwelt in Gilead in Bashan, and in her towns, and in all the s* → **amaute Gilead gam sung, Bashan gam leh a khuapite sung, leh Sharon gam**
- `[TDB77]` *And his brother Asaph, who stood on his right hand, even Asaph the son* → **Asaf in lasa pawl a nihna a makaipa hi a, ama khangsimna a pupi Levi l**
- `[TDB77]` *And when his armourbearer saw that Saul was dead, he fell likewise on * → **A galvan puapa in Saul a sihna a muh ciangin amah zong ama namsau tung**

## at

- `[TDB77]` *And they dwelt at Beer–sheba, and Moladah, and Hazar–shual,* → **Amaute Beersheba, Moladah, Hazarshual,**
- `[TDB77]` *And at Bilhah, and at Ezem, and at Tolad,* → **Bilhah, Ezem, Tolad,**
- `[TDB77]` *And at Bethuel, and at Hormah, and at Ziklag,* → **Bethuel, Hormah, Ziklag,**
- `[TDB77]` *And at Gibeon dwelt the father of Gibeon; whose wife's name was Maacha* → **Gibeon khuapi a satpa Jeiel in Gibeon ah teeng a, ama zi’ minin Ma-aka**
...
- `[TDB77]` *Therefore came all the elders of Israel to the king to Hebron; and Dav* → **Tua ahih ciangin Israel upate khempeuh kumpipa kiang Hebron ah hong pa**
- `[TDB77]` *And he appointed certain of the Levites to minister before the ark of * → **Tua tham loin amah in Topa’ thuciamna singkuang mai-ah na semin thu ng**
- `[TDB77]` *Benaiah also and Jahaziel the priests with trumpets continually before* → **siampite Benaiah leh Jahaziel-te in Pasian’ thuciamna singkuang mai-ah**
- `[TDB77]` *Fear before him, all the earth: the world also shall be stable, that i* → **leitung mi khempeuh aw, ama mai-ah ling un; hi hi, leitung kip a, kilo**
...
- `[TDB77]` *And the children of Gad dwelt over against them, in the land of Bashan* → **Bashan gam Salekah dongah Reubente’ gal khatah a teeng Gad minamte in:**
- `[TDB77]` *And Mikloth begat Shimeah. And these also dwelt with their brethren in* → **leh Mikloth (amah in Shimeah’ pa) ahi hi. Tu-in amau’ suan leh khakte **
- `[TDB77]` *And Mikloth begat Shimeam. And they also dwelt with their brethren at * → **tua ciangin Mikloth in Shifeam’ pa ahi hi; amau’ suan leh khakte zong **
- `[TDB77]` *And the battle went sore against Saul, and the archers hit him, and he* → **Gal kidona in Saul neh mahmah a, thaltawite in amah pha hi; thaltawite**